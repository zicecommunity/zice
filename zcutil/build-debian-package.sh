...
PACKAGE_VERSION=$($SRC_PATH/src/ziced --version | grep version | cut -d' ' -f4 | tr -d v)
...
# Copy binaries
cp $SRC_PATH/src/ziced $DEB_BIN
cp $SRC_PATH/src/zice-cli $DEB_BIN
cp $SRC_PATH/zcutil/fetch-params.sh $DEB_BIN/zice-fetch-params
# Copy docs
...
# Copy manpages
cp $SRC_DOC/man/ziced.1 $DEB_MAN
cp $SRC_DOC/man/zice-cli.1 $DEB_MAN
cp $SRC_DOC/man/zice-fetch-params.1 $DEB_MAN
# Copy bash completion files
cp $SRC_PATH/contrib/ziced.bash-completion $DEB_CMP/ziced
cp $SRC_PATH/contrib/zice-cli.bash-completion $DEB_CMP/zice-cli
# Gzip files
gzip --best -n $DEB_DOC/changelog
gzip --best -n $DEB_DOC/changelog.Debian
gzip --best -n $DEB_MAN/ziced.1
gzip --best -n $DEB_MAN/zice-cli.1
gzip --best -n $DEB_MAN/zice-fetch-params.1

cd $SRC_PATH/contrib

# Create the control file
dpkg-shlibdeps $DEB_BIN/ziced $DEB_BIN/zice-cli
