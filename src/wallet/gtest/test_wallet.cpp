...
CWalletTx GetValidSproutReceive(const libzice::SproutSpendingKey& sk, CAmount value, bool randomInputs, int32_t version = 2) {
    return GetValidSproutReceive(*params, sk, value, randomInputs, version);
}

CWalletTx GetInvalidCommitmentSproutReceive(const libzice::SproutSpendingKey& sk, CAmount value, bool randomInputs, int32_t version = 2) {
    return GetInvalidCommitmentSproutReceive(*params, sk, value, randomInputs, version);
}

libzice::SproutNote GetSproutNote(const libzice::SproutSpendingKey& sk,
                       const CTransaction& tx, size_t js, size_t n) {
    return GetSproutNote(*params, sk, tx, js, n);
}

CWalletTx GetValidSproutSpend(const libzice::SproutSpendingKey& sk,
                        const libzice::SproutNote& note, CAmount value) {
...
TEST(WalletTests, SetSaplingNoteAddrsInCWalletTx) {
    ...
    libzice::SaplingNote note(pk, 50000);
    auto cm = note.cm().get();
...
TEST(WalletTests, CheckSproutNoteCommitmentAgainstNotePlaintext) {
    ...
    ASSERT_THROW(wallet.GetSproutNoteNullifier(
        ...
        address,
        dec,
        hSig, 1), libzice::note_decryption_failed);
}
...
TEST(WalletTests, GetConflictedSaplingNotes) {
    ...
    auto maybe_pt = libzice::SaplingNotePlaintext::decrypt(
...
TEST(WalletTests, SetBestChainIgnoresTxsWithoutShieldedData) {
    ...
    mtxSapling.vShieldedOutput[0].cv = libzice::random_uint256();
    ...
    mtxSaplingTransparent.vShieldedOutput[0].cv = libzice::random_uint256();
