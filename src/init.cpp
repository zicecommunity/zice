...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "zice.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "ziced.pid"));
#endif
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 9733, 19733));
...
)
{
    ...
    pziceParams = ZCJoinSplit::Prepared(vk_path.string(), pk_path.string());
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    if (mapArgs.count("-migrationdestaddress")) {
        ...
        libzice::PaymentAddress address = DecodePaymentAddress(migrationDestAddress);
        if (boost::get<libzice::SaplingPaymentAddress>(&address) == nullptr) {
    ...
    if (GetBoolArg("-savesproutr1cs", false)) {
        ...
        pziceParams->saveR1CS(r1cs_path.string());
    }
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK)
        {
            ...
            else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
            {
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of ZiCE") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE)
            {
                strErrors << _("Wallet needed to be rewritten: restart ZiCE to complete") << "\n";
