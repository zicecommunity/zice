...
Jack Grigg (22):
      ...
      Instruct users to run zice-fetch-params if network params aren't available
      ...
      Throw an error if zice.conf is missing
      Show a friendly message explaining why ziced needs a zice.conf
      Fix gtest ordering broken by #1949
      Debian package lint
      Generate Debian control file to fix shlibs lint
      Create empty zice.conf during performance measurements
      Create empty zice.conf during coverage checks
...
Simon Liu (13):
      ...
      Remove UTF-8 BOM efbbbf from zice.conf to avoid problems with command line tools
      Closes #1097 so zice-cli now displays license info like ziced.
